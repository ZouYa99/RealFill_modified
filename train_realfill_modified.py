...
coco_path = "/home/master/zhoujian/Documents/lxd/data_sample/annotations/data_sample_108_trainval_sliced.json_coco.json"
...
class RealFillDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image_name = self.train_images_path[index].name  # get the image name(remove the images root path),in order to get bbox corresponding to the generation area
        image = exif_transpose(image)  # 解决自动旋转和镜像翻转的问题

        if not image.mode == "RGB":
            image = image.convert("RGB")

        if index < len(self) - 1:
            weighting = Image.new("L", image.size) # 创建出的图像为纯黑图
        else:
            weighting = Image.open(self.target_mask)
            weighting = exif_transpose(weighting)
        
        image, weighting = self.transform(image, weighting)
        example["images"], example["weightings"] = image, weighting < 0 # 像素值小于零的部分被设置为白色（True）
...
def main(args):
    ...
    vae.requires_grad_(False)  # vae frozen
